...
class ServiceCategory
{
    ...
    // --- View All Service Categories (Paginated) ---
    public static function getPaginatedCategories($perPage = 10, $offset = 0) {
        $db = Database::getPDO();
        $stmt = $db->prepare("SELECT * FROM service_categories ORDER BY category_id ASC LIMIT :limit OFFSET :offset");
    ...
    // --- Search Service Categories (Paginated) ---
    public static function searchServiceCategoryPaginated($searchQuery, $perPage, $offset) {
        $db = Database::getPDO();
        $serviceCategories = [];
        if (is_numeric($searchQuery)) {
            $stmt = $db->prepare("SELECT * FROM service_categories WHERE category_id = :search ORDER BY category_id ASC LIMIT :limit OFFSET :offset");
            $stmt->bindValue(':search', (int)$searchQuery, PDO::PARAM_INT);
        } else {
            $stmt = $db->prepare("SELECT * FROM service_categories WHERE LOWER(category_name) LIKE :search ORDER BY category_id ASC LIMIT :limit OFFSET :offset");
    ...
    public static function countSearchServiceCategory($searchQuery) {
        $db = Database::getPDO();
        if (is_numeric($searchQuery)) {
            $stmt = $db->prepare("SELECT COUNT(*) FROM service_categories WHERE category_id = :search");
            $stmt->bindValue(':search', (int)$searchQuery, PDO::PARAM_INT);
        } else {
            $stmt = $db->prepare("SELECT COUNT(*) FROM service_categories WHERE LOWER(category_name) LIKE :search");
            $searchTerm = '%' . strtolower($searchQuery) . '%';
            $stmt->bindValue(':search', $searchTerm, PDO::PARAM_STR);
        }
        $stmt->execute();
